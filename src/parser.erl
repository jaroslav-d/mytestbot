...
parse(Body, getUpdates) ->
  ...
  case Updates of
    [[{_, _}|[{<<"message">>, _}]]|_] -> parseUpdateText(Updates);
    [[{_, _}|[{<<"callback_query">>, _}]]|_] -> parseUpdateInlineKBoard(Updates);
    _Else -> parseUpdateDif(Updates)
  end;
...
parseUpdateInlineKBoard([Head|_Tail]) ->
  {_, Uid} = findRec(Head, <<"update_id">>),
  Chat = findRec(Head, <<"chat">>),
  {_, Cid} = findRec(Chat, <<"id">>),
...
findRec({Key, _}, TargetKey) when TargetKey =/= Key -> not_found;
findRec([], _) -> not_found;
findRec([Head|Tail], TargetKey) ->
  case findRec(Head, TargetKey) of
    not_found -> findRec(Tail, TargetKey);
    Else -> Else
  end.
